...
Ngoại trừ câu hỏi cuối cùng về địa điểm tổ chức (Hà Nội, Hồ Chí Minh...) vì địa điểm này đã được chốt và thiết lập cố định trên trang web (Trung tâm đào tạo Circle K, Hồ Chí Minh, ngày 04/07/2026). Dữ liệu đăng ký được thu thập và lưu trữ vào CRM Google Sheets thông qua hệ thống Apps Script Webhook.
...
- File `register.html` hiện tại chỉ có một số trường cơ bản và cần được cấu trúc lại giao diện (form layout) để hiển thị đầy đủ, gọn gàng, và trực quan các trường câu hỏi trên.
...
1. **Cập nhật `register.html`**:
   ...
   - Thêm hộp thông tin thanh toán (`payment-info`) mẫu để hiển thị học phí 300.000 VNĐ chuyển khoản tới HSBC - Nguyễn Ngọc Khánh Ngân.
