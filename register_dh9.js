...
function ensurePaidModal() {
    ...
    modal.innerHTML = `
        <div style="width:min(520px, 100%); background:#ffffff; color:#111827; border-radius:16px; padding:28px; box-shadow:0 24px 70px rgba(0,0,0,0.28); text-align:center;">
            ...
            <p style="margin:0 0 18px; color:#374151; line-height:1.55;">Chúc mừng bạn! Hệ thống đã ghi nhận thanh toán thành công. Bạn có thể tham gia nhóm Zalo DHM9 Hà Nội để nhận thông báo, lịch trình và kết nối với CultureCode Team.</p>
    ...
    for (let attempt = startAttempt; attempt < JSONP_MAX_ATTEMPTS; attempt++) {
        ...
        try {
            ...
            if (result.error === 'NOT_FOUND') {
                ...
                showInlineError('Số điện thoại này đang có nhiều đăng ký. CultureCode Team sẽ liên hệ để xử lý thủ công. Vui lòng không đăng ký lại.');
                return;
