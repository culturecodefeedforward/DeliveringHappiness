...
function handleRegistration(data, laneKey) {
  ...
  enqueueEmail(ss, uuid, 'BTC', BTC_EMAILS.join(','), 'Thông báo đăng ký mới - ' + lane.titleShort, lane.laneKey);
...
function handleSePayWebhook(body, laneKey) {
  ...
  if (matched.length === 0) {
    ...
    enqueueEmail(ss, m.uuid, PAYMENT_BTC_EMAIL_TYPE, BTC_EMAILS.join(','), 'Thanh toán xác nhận - ' + lane.titleShort, lane.laneKey);
